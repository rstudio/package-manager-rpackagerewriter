...
func (g *V2V3FilePathGetter) GetFilePath(outputDir string, arch *archive.ArchiveResults) string {
	return filepath.Join(outputDir, arch.OriginalChecksum+".tar.gz")
}

func (g *V2V3FilePathGetter) GetReadmePath(outputDir string, arch *archive.ArchiveResults) string {
	ext := ".readme"
