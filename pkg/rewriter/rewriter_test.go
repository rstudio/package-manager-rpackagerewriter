...
func (s *RewriterSuite) TestNewArchiveRewriter() {
	...
	s.Require().Nil(err)
	r := NewArchiveRewriter("outputDir", "readmeDir", dir, fpg, 256, 6)
	s.Require().Equal(&archiveRewriter{
		OutputDir:       "outputDir",
...
func (s *RewriterSuite) TestArchiveRewriterRewrite() {
	...
	fpg, err := utils.NewFilePathGetterFactory().GetFilePathGetter(1)
	s.Require().Nil(err)
	rewriter := NewArchiveRewriter(dir, readmeDir, dir, fpg, 1024*2, 6)
...
func (s *RewriterSuite) TestArchiveRewriterGetNoReadme() {
	...
	fpg, err := utils.NewFilePathGetterFactory().GetFilePathGetter(1)
	s.Require().Nil(err)
	rewriter := NewArchiveRewriter("", readmeDir, dir, fpg, 256, 6)
	f, err := os.Open("../testdata/adhoc_1.1.tar.gz")
	s.Require().Nil(err)
...
func (s *RewriterSuite) TestArchiveRewriterRewriteReadme() {
	...
	s.Require().Nil(err)
	rewriter := NewArchiveRewriter(dir, readmeDir, dir, fpg, 256, 6)
...
func (s *RewriterSuite) TestArchiveRewriterRewriteBinaryTarNoDescription() {
	...
	s.Require().Equal(true, errors.Is(err, RewriteError{}))
}
